...
def load_external_data(config: Config, source: Config.External.Source) -> DataFrame:
  ...
  logger.debug("format = %s", source.format)
  ...
  try:
    ...
    exception = LoadExternalDataError(reason=f"FileNotFoundError = {type(e).__name__}", format=source.format, path=source.path)
    logger.error(exception.message)
    raise exception from e
  except IsADirectoryError as e:
    logger.warning("failed to load external data")
    exception = LoadExternalDataError(reason=f"IsADirectoryError = {type(e).__name__}", format=source.format, path=source.path)
    logger.error(exception.message)
    raise exception from e
  except PermissionError as e:
    logger.warning("failed to load external data")
    exception = LoadExternalDataError(reason=f"PermissionError = {type(e).__name__}", format=source.format, path=source.path)
    logger.error(exception.message)
    raise exception from e
  except OSError as e:
    logger.warning("failed to load external data")
    exception = LoadExternalDataError(reason=f"OSError = {type(e).__name__}", format=source.format, path=source.path)
    logger.error(exception.message)
    raise exception from e
